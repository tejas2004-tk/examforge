...
export function ResetPasswordPage() {
  ...
  if (done) {
    return (
      <div className="card">
        <h2 className="text-2xl font-bold text-ink">Password reset</h2>
        <p className="mt-2 text-sm text-ink-muted">Your password has been updated successfully.</p>
  ...
  if (!token) {
    return (
      <div className="card">
        <h2 className="text-2xl font-bold text-ink">Invalid link</h2>
        <p className="mt-2 text-sm text-ink-muted">This password reset link is missing its token. Please request a new one.</p>
  ...
  return (
    <div className="card">
      ...
      <p className="mt-1 text-sm text-ink-muted">Choose a new, strong password.</p>
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div>
          ...
          {errors.newPassword && <p className="mt-1 text-xs text-critical-ink">{errors.newPassword.message}</p>}
        </div>

        <div>
          <label className="label" htmlFor="confirm">Confirm password</label>
          <input id="confirm" type="password" className="input" placeholder="••••••••" {...register('confirm')} />
          {errors.confirm && <p className="mt-1 text-xs text-critical-ink">{errors.confirm.message}</p>}
        </div>
