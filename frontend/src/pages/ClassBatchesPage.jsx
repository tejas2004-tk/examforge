            ...
            <div key={b.id} className="card cursor-pointer hover:shadow-md transition-shadow" onClick={() => openDetail(b)}>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">{b.course?.name ?? 'No course'}</p>
                </div>
                <button onClick={(e) => { e.stopPropagation(); onDelete(b.id); }} className="text-slate-400 hover:text-red-500">
      ...
      <Modal open={showDetail} onClose={() => { setShowDetail(false); setSelectedBatch(null); }} title={selectedBatch?.name ?? 'Batch Detail'} width="max-w-2xl">
        {selectedBatch && (
          <div className="space-y-4">
            <div className="text-sm text-slate-500">Course: {selectedBatch.course?.name ?? 'None'}</div>
            <div>
              ...
              {searchResults.length > 0 && (
                <div className="mt-2 max-h-40 overflow-y-auto rounded-lg border border-slate-200">
                  {searchResults.map((s) => (
                    <button key={s.id} onClick={() => addStudents([s.id])} className="flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-slate-50">
                ...
                <div className="divide-y divide-slate-100 rounded-lg border border-slate-200">
                  {selectedBatch.students?.map((cs) => (
                    <div key={cs.id} className="flex items-center justify-between px-3 py-2">
                      ...
                      <button onClick={() => removeStudent(cs.studentId)} className="text-xs text-red-500 hover:text-red-700">Remove</button>
                    </div>
                  ))}
                </div>
              )}
